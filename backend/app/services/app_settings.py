...
def get_app_settings(db: Session) -> dict[str, dict[str, Any]]:
    ...
    legacy = output.pop("organizer_task", None)
    if legacy:
        output.setdefault("organizer_task_kibin", {})
        for key, value in legacy.items():
            output["organizer_task_kibin"].setdefault(key, value)
    return output
